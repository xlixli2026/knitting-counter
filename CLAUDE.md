...
For hosting (GitHub Pages) there are optional companion files — `manifest.webmanifest`, `sw.js` (offline cache, registered only over https; bump its `CACHE` name when assets change), `icon-180.png`/`icon-512.png` (regenerate with a script if the palette changes; navy square, light circle, teal plus) — but `index.html` must keep working alone via `file://` when they're absent.
