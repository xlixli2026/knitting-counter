...
For hosting (GitHub Pages) there are optional companion files — `manifest.webmanifest`, `sw.js` (offline cache, registered only over https; bump its `CACHE` name when assets change), `icon-180.png`/`icon-512.png` (regenerate via the scratchpad `make_icons.py` if the palette changes; `#222834` square, white yarn-ball-and-needles drawn from opaque-white SDF strokes) — but `index.html` must keep working alone via `file://` when they're absent. App name is "KnitCount" (title, manifest, apple-mobile-web-app-title).
